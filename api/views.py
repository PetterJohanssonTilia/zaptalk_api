...
class MovieFilter(filters.FilterSet):
    ...
    def search_movies(self, queryset, name, value):
        return queryset.filter(
            Q(title__icontains=value) | Q(extract__icontains=value)
            )
...
class MovieViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        followed_likes = (
            self.request.query_params.get('followed_likes', '')
            .lower() == 'true'
        )
    ...
    def random(self, request):
        ...
        if count == 0:
            return Response(
                {"detail": "No movies with thumbnails available"},
...
class UserProfileViewSet(viewsets.ModelViewSet):
    ...
    def me(self, request):
        ...
        if request.method == 'GET':
            ...
            logger.info(
                f"Received PUT request for user {request.user.username}"
            )
            ...
            try:
                ...
                if serializer.is_valid():
                    ...
                    logger.info(
                        f"Profile updated successfully: {updated_profile}"
                    )
                    return Response(serializer.data)
                else:
    ...
    def feed(self, request):
        ...
        comment_items = [
            dict(item, **{'type': 'comment'})
            for item in comments_serializer.data
        ]
        like_items = [
            dict(item, **{'type': 'like'})
            for item in likes_serializer.data
        ]
        feed_items = sorted(
                ...
                return Response(
                    {
                        "detail": (
                            ...
                            f"{user_to_follow.user.username}."
                        )
                    },
    ...
    def is_banned(self, request, pk=None):
        ...
        is_banned = (
            Ban.objects.filter(user=profile.user, is_active=True)
            .exists()
        )
...
class LikeViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        try:
            ...
            return Response(
                {"detail": "An error occurred while fetching likes"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class BanViewSet(viewsets.ModelViewSet):
    ...
    def unban_user(self, request):
        ...
        if not active_bans.exists():
            return Response(
                {"message": f"No active ban found for user {username}"},
                status=status.HTTP_404_NOT_FOUND
            )
        ...
        active_appeals = (
            BanAppeal.objects
            .filter(ban__user=user, is_resolved=False)
        )
        ...
        return Response(
            {
                "message": (
                    f"User {username} has been unbanned successfully and "
                    "all ban appeals have been resolved"
                )
            },
            status=status.HTTP_200_OK
        )
