...
class LikeSerializer(serializers.ModelSerializer):
    ...
    # This is used for the feed page
    # To get a like or a comment "Sandra Commented on x"
    ...
    def get_movie_title(self, obj):
        if obj.content_type == Movie.get_default_like_content_type():
            ...
            return (
                obj.content_object.movie.title
                if obj.content_object.movie
                else None
            )
        return None
...
class CommentSerializer(serializers.ModelSerializer):
    ...
    def get_movie_details(self, obj):
        if obj.movie:
            return {
                ...
                'thumbnail': (
                    obj.movie.thumbnail
                    if hasattr(obj.movie, 'thumbnail')
                    else None
                )
            }
        return None
    ...
    def to_representation(self, instance):
        ...
        if user_representation and 'profile' in user_representation:
            user_representation['avatar'] = (
                user_representation['profile'].get('avatar')
            )
...
class BanSerializer(serializers.ModelSerializer):
    ...
    user_id = serializers.IntegerField(source='user.id', read_only=True)
    user_username = (
        serializers.CharField(source='user.username', read_only=True)
    )
    banned_by_username = (
        serializers.CharField(source='banned_by.username', read_only=True)
    )
    banned_by_id = (
        serializers.IntegerField(source='banned_by.id', read_only=True)
    )
...
class BanAppealSerializer(serializers.ModelSerializer):
    ...
    user_username = serializers.CharField(
        source='ban.user.username', read_only=True
    )
    ...
    def create(self, validated_data):
        ...
        if not active_ban:
            raise serializers.ValidationError(
                f"No active ban found for user '{username}'."
            )
